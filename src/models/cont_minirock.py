...
tsfr = RandomForestRegressor(n_estimators = 5000, n_jobs=-1, random_state=67)
    ...
    for (_,_,files) in os.walk(DATA_PATH):
        for file in files:
            if file.endswith('.csv'):
                ...
                transf = MiniRocketMultivariate()
                #transf = Catch22(replace_nans=True)
                #transf = RandomShapeletTransform()
                X = transf.fit_transform(X).astype(np.float32)
                print(X.shape)
